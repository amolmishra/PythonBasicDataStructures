class EmptyHashTableError(Exception):
    """
...
class HashTable:
    ...
    def __init__(self):
        """
            ...
            We use lists to implement the HashTable
    ...
    def put(self, key, value):
        ...
        :param self: the object's reference
        :param key: used to calculate the hash
        :param value: the value mapped against the key
        """
        # if table is 75% or more utilised double its size
    ...
    def _findslot(self, slot, key):
        ...
        :param key: key we're trying to put or get
        :return: next slot where the key would fit
        """
        next_slot = self.hash_function(slot + 1)

        while self._table[slot] is not None and self._table[next_slot][0] != key:
    ...
    def utilization(self):
        return float(len(self) / self._size)
